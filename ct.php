...
(function(){
  ...
  document.addEventListener('click',function(e){var t=e.target.closest('a,button');if(!t)return;var href=(t.getAttribute('href')||'').toLowerCase();var txt=(t.textContent||'').trim().toLowerCase();var action='';if(href.indexOf('buygoods.com/secure/upsell')>-1||/yes|accept|upgrade|add|buy/i.test(txt))action='accept';else if(/no thank|skip|decline|no,? thank/i.test(txt))action='decline';if(!action)return;var clickHref=t.getAttribute('href')||'';var cOid='';var clickAmt=0;var clickPack='';try{var cp=new URLSearchParams(clickHref.split('?')[1]||'');cOid=cp.get('order_id')||cp.get('order_id_global')||'';clickAmt=parseFloat(cp.get('amount')||cp.get('total')||cp.get('price'))||0;}catch(e2){}/* Try to extract amount from link text or nearby text */if(!clickAmt){var pm=txt.match(/\$\s*([\d.]+)/);if(pm)clickAmt=parseFloat(pm[1])||0;}if(!clickAmt){try{var par=t.closest('div,li,td,section');if(par){var ptxt=par.textContent||'';var ppm=ptxt.match(/\$\s*([\d.]+)/);if(ppm)clickAmt=parseFloat(ppm[1])||0;}}catch(e3){}}/* Try to detect pack name from nearby text */try{var packEl=t.closest('div,li,td,section');if(packEl){var ptxt2=(packEl.textContent||'').trim();var bm=ptxt2.match(/(\d+)\s*(?:bottle|pack|box|month|supply)/i);if(bm)clickPack=bm[0].trim();}}catch(e4){}var d={num:unum,action:action,ts:Date.now()};if(orderId)d.orderId=orderId;if(cOid)d.clickOrderId=cOid;if(clickAmt)d.amount=clickAmt;if(clickPack)d.pack=clickPack;fetch(FB+'/tracker/upsell-events/'+sid+'/'+unum+'.json',{method:'PATCH',body:JSON.stringify(d),keepalive:true});if(sk){var p={};p['upsell'+unum]=action;if(clickAmt)p['upsell'+unum+'Amount']=clickAmt;if(clickPack)p['upsell'+unum+'Pack']=clickPack;if(orderIdGlobal)p.orderIdGlobal=orderIdGlobal;if(orderIdLocal)p['upsell'+unum+'OrderId']=orderIdLocal;if(cOid&&cOid!==orderIdLocal)p['upsell'+unum+'ClickOrderId']=cOid;for(var _ui=1;_ui<unum;_ui++){var _prev=null;try{_prev=localStorage.getItem('_ct_up'+_ui+'_oid');}catch(e){}if(_prev)p['upsell'+_ui+'OrderId']=_prev;}fetch(FB+'/tracker/sales/'+sk+'.json',{method:'PATCH',body:JSON.stringify(p),keepalive:true}).then(function(){if(action==='accept'){fetch(FB+'/tracker/sales/'+sk+'.json').then(function(r){return r.json();}).then(function(fullSale){if(fullSale){fullSale._upsellNotify=true;fullSale._upsellNum=unum;_notifySale(fullSale);}}).catch(function(){});}});}});
})();
